...
NETWORK=$(grep -s '^NET_NAME=' "$(dirname "$0")/../.env" | cut -d= -f2)
NETWORK=${NETWORK:-eth-devnet-network}
...
for i in 1 2 3 4 5; do
  ...
  out=$(docker run --rm --network "$NETWORK" "$FOUNDRY_IMAGE" \
    "cast send $TO --value 0.${i}ether --private-key $KEY --rpc-url $RPC --legacy")
  echo "$out" | grep -E 'status|blockNumber' || true
